...
#define MAX_SCTE_ARRAY_SIZE 255
...
typedef struct
{
	unsigned long	splice_event_id;
	char			splice_event_cancel_indicator;
	char			out_of_network_indicator;
	char			program_splice_flag;
	char			splice_flag;
	char			duration_flag;
	char			splice_immediate_flag;
	splice_time_t	splice_time;

	unsigned char	component_count;
	unsigned char	component_tag[MAX_SCTE_ARRAY_SIZE];
	splice_time_t	splices[MAX_SCTE_ARRAY_SIZE];

	break_duration_t break_duration;

	unsigned short	unique_program_id;
	unsigned char	avail_num;
	unsigned char	avail_expected;
}scte35_splice_insert_t;
...
@return		ZIXI_WARNING_OVER_LIMIT
*/
DLL_EXPORT int zixi_inject_scte35(void *stream_handle, scte35_splice_insert_t scte35_splice_command);
...
@return		ZIXI_WARNING_OVER_LIMIT
*/
DLL_EXPORT int zixi_inject_scte35_to_multiplexed_stream(void *stream_handle, int index, scte35_splice_insert_t scte35_splice_command);
